...
$stdout = Join-Path $env:TEMP ("plant-downtime-root-cause-hub-" + [guid]::NewGuid().ToString() + "-stdout.log")
$stderr = Join-Path $env:TEMP ("plant-downtime-root-cause-hub-" + [guid]::NewGuid().ToString() + "-stderr.log")
$port = "5474"
$process = $null
$edgeCandidates = @(
...
try {
    ...
    $targets = @(
        ...
        @{ Url = "http://127.0.0.1:$port/incident-lane"; File = "02-incident-lane-proof.png"; Size = "1600,1320" },
        @{ Url = "http://127.0.0.1:$port/root-cause-map"; File = "03-root-cause-map-proof.png"; Size = "1600,1220" },
        @{ Url = "http://127.0.0.1:$port/restart-posture"; File = "04-restart-posture-proof.png"; Size = "1600,1220" }
    )
